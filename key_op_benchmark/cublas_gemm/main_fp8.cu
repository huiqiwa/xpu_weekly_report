/**
 * cuBLAS FP8 (E4M3) GEMM benchmark
 * Usage: ./gemm_fp8 [gpu_id] [M] [K] [N] [iters] [warmup]
 */
#include "gemm_fp8.h"
...
int main(int argc, char** argv) {
    ...
    size_t size_A = (size_t)cfg.M * cfg.K;  // 1 byte per element
    size_t size_B = (size_t)cfg.K * cfg.N;
    ...
    CublasLtContext ctx;
    init_cublaslt_fp8(ctx, cfg.M, cfg.N, cfg.K);

    for (int i = 0; i < cfg.warmup; i++)
        run_gemm_fp8(ctx, cfg.M, cfg.N, cfg.K, d_A, d_B, d_C, d_scaleA, d_scaleB);
