 ...
 * Usage: ./gemm_int8 [gpu_id] [M] [K] [N] [iters] [warmup]
 */
#include "common.h"
...
int main(int argc, char** argv) {
    ...
    size_t size_A = (size_t)cfg.M * cfg.K * sizeof(int8_t);
    size_t size_B = (size_t)cfg.K * cfg.N * sizeof(int8_t);
    size_t size_C = (size_t)cfg.M * cfg.N * sizeof(int32_t);
    ...
    for (int i = 0; i < cfg.warmup; i++)
        run_gemm_int8(handle, cfg.M, cfg.N, cfg.K, d_A, d_B, d_C);
