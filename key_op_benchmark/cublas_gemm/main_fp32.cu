/**
 * cuBLAS FP32/TF32 GEMM benchmark
 * Usage: ./gemm_fp32 [gpu_id] [M] [K] [N] [iters] [warmup] [tf32]
 *   tf32: 0 or 1 (default: 0)
 */
#include "common.h"
...
int main(int argc, char** argv) {
    ...
    if (argc > 7) { use_tf32 = atoi(argv[7]) != 0; cfg.dtype = TF32; }
    ...
    size_t size_A = (size_t)cfg.M * cfg.K * sizeof(float);
    size_t size_B = (size_t)cfg.K * cfg.N * sizeof(float);
    size_t size_C = (size_t)cfg.M * cfg.N * sizeof(float);
    ...
    for (int i = 0; i < cfg.warmup; i++)
        run_gemm_fp32(handle, cfg.M, cfg.N, cfg.K, d_A, d_B, d_C, use_tf32);
