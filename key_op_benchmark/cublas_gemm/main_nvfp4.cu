/**
 * cuBLAS NVFP4 GEMM benchmark
 * Usage: ./gemm_nvfp4 [gpu_id] [M] [K] [N] [iters] [warmup]
 */
...
int main(int argc, char** argv) {
    ...
    size_t size_A = ((size_t)cfg.M * cfg.K + 1) / 2;
    size_t size_B = ((size_t)cfg.N * cfg.K + 1) / 2;
    ...
    CublasLtFP4Context ctx;
    init_cublaslt_nvfp4(ctx, cfg.M, cfg.N, cfg.K);

    for (int i = 0; i < cfg.warmup; i++)
        run_gemm_nvfp4(ctx, cfg.M, cfg.N, cfg.K, d_A, d_B, d_C);
