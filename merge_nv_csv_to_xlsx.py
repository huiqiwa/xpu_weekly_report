...
def resolve_report_csv_base(base_dir):
    gpu_root = os.path.join(base_dir, "GPU")
    candidates = [path for path in sorted(glob.glob(os.path.join(glob.escape(gpu_root), "*"))) if os.path.isdir(path)]
    if not candidates:
        raise FileNotFoundError(f"No device report directory found under: {gpu_root}")
    if len(candidates) > 1:
        raise ValueError(
            "Multiple device report directories found under GPU; please keep only one target device directory."
        )
    return candidates[0]
